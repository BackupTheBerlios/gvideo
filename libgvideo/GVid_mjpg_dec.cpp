...
//-------------------- (M)jpeg decompression

int GVMjpgDec::getbyte()
{
	return *datap++;
};

int GVMjpgDec::getword()
{
    int c1, c2;
    c1 = *datap++;
    c2 = *datap++;
...
*/
int GVMjpgDec::readtables(int till, int *isDHT)
{
...
*/
int GVMjpgDec::decode(UINT8 **frame_data, UINT8 *raw_data, int width, int height)
{
    ...
    and pic not allocated alloc the good size and mark the change 
    ...
    if (intwidth != width || intheight != height || *frame_data == NULL) 
    {
        ...
        *frame_data = new UINT8 [intwidth * (intheight + 8) * 2];
    }
    ...
    switch (dscans[0].hv) 
    {
        case 0x22: // 411
            ...
            ypitch = 16 * pitch;
            convert_index = 1;// yuv420pto422; //choose the right conversion function
            break;
        case 0x21: //422
            mb=4;
            mcusx = width >> 4;
            ...
            pitch = width * bpp; // YUYV out
            ypitch = 8 * pitch;
            convert_index = 2; //yuv422pto422; //choose the right conversion function
            break;
        case 0x11: //444
            mcusx = width >> 3;
            mcusy = height >> 3;
            ...
            {
                mb = 1;
                convert_index = 3; //yuv400pto422; //choose the right conversion function
            }
            else 
            {
                mb=3;
                convert_index = 4; //yuv444pto422; //choose the right conversion function
            }
            break;
        default:
            err = ERR_NOT_YCBCR_221111;
    ...
    for (my = 0,y=0; my < mcusy; my++,y+=ypitch) 
    {
        for (mx = 0,x=0; mx < mcusx; mx++,x+=xpitch) 
        {
            ...
            {
                case 6: 
                    decode_mcus(decdata->dcts, mb, dscans, max);
            ...
            switch(convert_index)
            {
                case 1:
                    conversor->yuv420pto422(decdata->out, (*frame_data) + y + x, pitch);
                    break;
                case 2:
                    conversor->yuv422pto422(decdata->out, (*frame_data) + y + x, pitch);
                    break;
                case 3:
                    conversor->yuv400pto422(decdata->out, (*frame_data) + y + x, pitch);
                    break;
                 case 4:
                    conversor->yuv444pto422(decdata->out, (*frame_data) + y + x, pitch);
                    break;
            }
            //convert(decdata->out,frame_data+y+x,pitch); //convert mcu to 422
        }
    }
...
void GVMjpgDec::dec_makehuff(struct dec_hufftbl *hu, int *hufflen, UINT8 *huffvals)
{
...
int GVMjpgDec::fillbits( int le, unsigned int bi)
{
    int b, m;
...
int GVMjpgDec::dec_readmarker()
{
    int m;

    in->left = fillbits( in->left, in->bits);
    if ((m = in->marker) == 0)
        return 0;
    in->left = 0;
    in->marker = 0;
...
#define LEBI_GET()	(le = in->left, bi = in->bits)
#define LEBI_PUT()	(in->left = le, in->bits = bi)

#define GETBITS(n) (					\
  (le < (n) ? le = fillbits( le, bi ), bi = in->bits : 0),	\
  (le -= (n)),							\
  bi >> le & ((1 << (n)) - 1)					\
)
...
int GVMjpgDec::dec_rec2( struct dec_hufftbl *hu, int *runp, int c, int i )
{
    LEBI_DCL;
    ...
    {
        for (i = DECBITS;
            (c = ((c << 1) | GETBITS(1))) >= (hu->maxcode[i]); i++);
        if (i >= 16) 
        {
            in->marker = M_BADHUFF;
            return 0;
...
void GVMjpgDec::decode_mcus( int *dct, int n, struct scan *sc, int *maxp )
{
...
int GVMjpgDec::dec_checkmarker()
{
	int i;

	if (dec_readmarker() != info.rm)
		return -1;
	info.nm = info.dri;
...
*/
void GVMjpgDec::idct(int *input, int *out, int *quant, long off, int max)
{
    ...
    {
        t0 += input[0] * quant[0];     //only DC available
    ...
    {
        j = *zig2p++;
        t0 += input[j] * (long) quant[j];
        j = *zig2p++;
        t5 = input[j] * (long) quant[j];
        j = *zig2p++;
        t2 = input[j] * (long) quant[j];
        j = *zig2p++;
        t7 = input[j] * (long) quant[j];
        j = *zig2p++;
        t1 = input[j] * (long) quant[j];
        j = *zig2p++;
        t4 = input[j] * (long) quant[j];
        j = *zig2p++;
        t3 = input[j] * (long) quant[j];
        j = *zig2p++;
        t6 = input[j] * (long) quant[j];
...
void GVMjpgDec::idctqtab(UINT8 *qin, int *qout)
{
    int i, j;
